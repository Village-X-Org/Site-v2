...
$result = doUnprotectedQuery("SELECT pu_id, picture_filename, pu_description, donor_id, donor_email, project_id, project_name, pu_timestamp FROM project_updates JOIN donations ON pu_project_id=donation_project_id JOIN donors ON donor_id=donation_donor_id JOIN projects ON pu_project_id=project_id JOIN pictures ON pu_image_id=picture_id WHERE pu_timestamp>DATE_SUB(NOW(), INTERVAL 1 WEEK) AND donation_is_test=0 ORDER BY donor_id, project_id");
$emailBuffer = '';
$lastDonorId = 0;
$lastProjectId = 0;
