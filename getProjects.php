...
$result = doQuery("SELECT project_id, project_name, project_lat, project_lng, project_type, picture_filename, project_summary, GROUP_CONCAT(CONCAT(pu_image_id, ':', DATE_FORMAT(pu_timestamp, '%b %D, %Y'), ' ', IFNULL(pu_description,' ')) SEPARATOR '~') AS updatePictures, village_name, project_funded, project_budget FROM projects JOIN villages ON project_village_id=village_id JOIN project_updates ON pu_project_id=project_id JOIN pictures ON project_image_id=picture_id GROUP BY project_id ");
