<?php
	...
	print cached('projects.json', doJsonQuery("SELECT project_id, project_name, project_lat + (RAND() * .04 - .02) AS project_lat, 
		project_lng + (RAND() * .04 - .02) AS project_lng, project_village_id, project_type, project_budget, project_funded, (SELECT picture_filename FROM pictures WHERE picture_id=project_profile_image_id) AS picture_filename, 
		...
		FROM projects JOIN (SELECT pe_project_id, GROUP_CONCAT(DATE_FORMAT(pe_date, '%b %D') ORDER BY pet_id) AS dates, GROUP_CONCAT(pet_label ORDER BY pet_id) AS labels FROM project_event_types LEFT JOIN project_events ON pet_id=pe_type GROUP BY pe_project_id) AS timeline ON timeline.pe_project_id=project_id
